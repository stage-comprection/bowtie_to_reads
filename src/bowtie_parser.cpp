...
BowtieParser::BowtieParser(string& referenceGenomePath){
...
void BowtieParser::getReadsFromReference(string& outputFilePath, string& noAlign){
    ...
    while(cin.getline(line, 5000)){
        ...
        if(line[0] != '@'){
            ...
            if (splittedLine[2] != "*"){
                ...
                if (noAlign == "T" or noAlign == "True" or noAlign == "true" or noAlign == "TRUE"){
                    outputFile << ">" + name + "\n" + "not_aligned\n";
                }else {
                    outputFile << ">" + name + "\n" + splittedLine[4] + "\n";
                }
